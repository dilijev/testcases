...
const generateRandomDigits = (length: number): number[] => {
  ...
  return Array.from({ length } satisfies { length: number }, () =>
    randomDigit()
  );
};
...
const generateCardNumber = (
  ...
  const length =
    format.lengths[Math.floor(Math.random() * format.lengths.length)];
  const prefix =
    format.prefixes[Math.floor(Math.random() * format.prefixes.length)];
