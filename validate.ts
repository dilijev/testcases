...
const validateTestCases = (testCases: TestCase[]): void => {
  ...
  for (const testCase of testCases) {
    ...
    if (isValid === testCase.shouldBeValid) {
      ...
      failedTests++;
      console.error(
        `Test failed for ${testCase.cardType}: ${testCase.cardNumber}`
      );
      console.error(
        `  Expected validity: ${testCase.shouldBeValid}, Got: ${isValid}`
      );
    }
  }

  console.log(
    `Validation complete: ${passedTests} passed, ${failedTests} failed`
  );
  if (failedTests === 0) {
    console.log(
      "All test cases are valid! Ready to use in your testing suite."
    );
  }
};
